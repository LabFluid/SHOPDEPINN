...
# ╔═╡ fd1406e0-c8a2-11ef-34f7-3511d5feb230
begin
	using JLD
	Upwind_Dados = load("Upwind_Dados.jld")["Upwind_Dados"]
	xt_Dados = hcat(Upwind_Dados[:,1:2])'
	u_Dados = hcat(Upwind_Dados[:,3])'
end

# ╔═╡ 24a1d8b0-273d-484c-8e62-c5c1bc12b81d
begin
...
# ╔═╡ 2b1ad1a2-2050-419f-8c11-f28886ca95ef
md"$\begin{align}
...
# ╔═╡ 95a7376b-d289-49ae-90c4-3ffa17030f33
begin
	c(x) = 1/5+sin(x-1)^2
	f(x) = exp(-100(x-1)^2)
...
# ╔═╡ 8deb4714-a371-4124-bfbc-d98ca9bc6848
begin
	Neurons = 10
	Layers = 10
	Epoch = 1500
...
# ╔═╡ baf1c069-918b-4cd7-8a7c-8e390822774e
begin
Neural_Network = Chain(
    ...
    Weight_NN, layers = Lux.setup(MersenneTwister(seed), Neural_Network) 

    tstate = Training.TrainState(Neural_Network, Weight_NN, layers, Adam(0.001f0))
end

# ╔═╡ e79611df-cdc2-4a67-800d-550d35ac872f
...
# ╔═╡ 4648a5ea-5fdd-453f-858d-7737e11e002f
begin
	tstate_ = main(tstate, AutoZygote(), (xt_Dados, u_Dados), Epoch)
end

# ╔═╡ 39bf25b1-47c5-4b8f-87df-809d1185c521
let
    d = 0.005; x = 0:d:5; t = 0:d:6.4
	
	u_real = [u_exata(i, j)*aux(i, j) for j in t, i in x]
	u_rede = [dev_cpu(Lux.apply(tstate.model, dev_cpu([i, j]), tstate.parameters, tstate.states)[1])[1,1] for j in t, i in x ]
	...
	h2 = heatmap(x, t, u_rede, title="Rede Interpoladora", clim=(vmin, vmax))
	h3 = heatmap(x, t, u_erro, title = "Erro")
	heatmap(h1, h2, h3)
end

# ╔═╡ e1911892-2a0e-4f69-afbd-093498e9bbde
let
    d = 0.01
	x = 0:d:5  
    t = 0:d:6.4 
    u_rede = [dev_cpu(Lux.apply(tstate.model, dev_cpu([i, j]), tstate.parameters, 				  tstate.states)[1])[1,1] for j in t, i in x ]
	save("Rede_Ajustada.jld", 
		"Neurons", Neurons,
		"Layers", Layers,
		"Weight_NN", Weight_NN)
end
...
manifest_format = "2.0"
project_hash = "22850f01920c7955afaa6f40fed60497f3ec1376"
